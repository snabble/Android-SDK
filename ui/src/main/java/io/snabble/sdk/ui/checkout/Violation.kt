...
fun List<ViolationNotification>.getMessage(context: Context) = joinToString("\n") {
    ...
    when (it.type) {
        ...
        "coupon_currently_not_valid" -> res.getString(
            I18nUtils.getIdentifier(
                res,
                R.string.Snabble_Violations_couponCurrentlyNotValid
            ), it.name
        )

        "coupon_already_voided" -> res.getString(
            I18nUtils.getIdentifier(
                res,
                R.string.Snabble_Violations_couponAlreadyVoided
            ), it.name
        )
